        ...
        *--------------------------------------------*/
        ...
        public function admin_init() 
        {
            ...
            if ( defined( 'Q_PLUGIN_DEBUG' ) && Q_PLUGIN_DEBUG === true ) {
                #$q_support_options["api_url"] = 'http://localhost/q/www/api/'; // local testing ##
            }
...
?>
                ...
                printf( 
                    ...
                    ,esc_url( Q_SUPPORT_DOMAIN.'releases/q-support/#terms-of-use' ) // terms ##
        ...
        **/
        public static function ajax_q_support_form() 
        {
            ...
            if ( 
                ...
                if ( $user && is_user_logged_in() && $user->user_login && $user->user_email ) {
                    ...
                    // store it ##
                    if ( Q_PLUGIN_DEBUG ) { update_option( "q_support_question_post", $q_dump ); }
                        ...
                        if ( isset( $results->status ) && $results->status === true ) {
                            ...
                            if ( ! isset( $results->key ) ) {
                                ...
                                $return = $results->response.' - '.sprintf( 
                                            ...
                                            ,esc_url( Q_SUPPORT_DOMAIN.'support/question/'.$question_url )
                            ...
                            // store it ##
                            Q_Plugin::$store["ajax_q_support_form"] = array( 
                                    'q_support_form__status' => 'false' 
                                ,   'q_support_form__result' => var_export($result)
                            );
        ...
        public function add_help_tab() 
        {
            ...
            $this->help_tabs = array(
                ...
                'q_support' => array(
                    'title'   => __( 'Q Support', Q_Support::$text_domain )
                )
        ...
        public function admin_bar_menu( $admin_bar )
        {
            ...
            // not clearing totals, so subtract new from old ##
                ...
                $update = sprintf(
                        __(
                                'Q Support:<span> %s %s</span>'
                            ,   Q_Support::$text_domain
                        )
                        , "<span class='count count-answers count-".$answers_new."' title='{$answers_new} Answers'>{$answers_new}</span>"
                        , "<span class='count count-comments count-".$comments_new."' title='{$comments_new} Comments'>{$comments_new}</span>"
                    );
        ...
        public function register_post_type() 
        {
            ...
            $labels = array(
                ...
                ,'parent_item_colon'        => ''
                ,'menu_name'                => __('Q Support', Q_Support::$text_domain )
            );
        ...
        public function post_row_actions( $actions, $post )
        {
            ...
            if ( $post->post_type == "q_support" ) {
                ...
                if ( $question_url ) {
                    ...
                    $actions['view'] = '<a href="'.Q_SUPPORT_DOMAIN.'support/question/'.$question_url.'" class="_blank">View</a>';
        ...
        public function manage_columns_answers( $text, $post_id, $column, $column_name )
        {
            ...
            $answers = get_post_meta( $post_id, 'q_support_question_answers' ) ? $resolved.get_post_meta( $post_id, 'q_support_question_answers', true ) : 0 ;
            
            // Add a tick here if the question has an accepted answer ##
            $resolved = false;
            $resolved = get_post_meta( $post_id, 'q_support_question_resolved', true );
            if ( $resolved ) {
                
                $answers = '<span class="accepted">'.$answers.'</span>';
        ...
        public function manage_columns_votes( $text, $post_id, $column, $column_name )
        {
            ...
            return sprintf(
                "%s %s"
                , '<span class="q_support_icons thumb_up" title="'.__("Up Votes", Q_Support::$text_domain ).'">'.$votes_up.'</span>'
                , '<span class="q_support_icons thumb_down" title="'.__("Down Votes", Q_Support::$text_domain ).'">'.$votes_down.'</span>'
            );
        ...
        public function manage_columns_submitted( $text, $post_id, $column, $column_name )
        {
            ...
            return ( $q_support_submitted ) ? '<span class="q_support_icons flag_green" title="'.__("Connected to Q Support").'"></span>' : '<span class="flag_red" title="'.__("Not connected to Q Support").'"></span>' ;
        ...
        public function admin_update_notice() 
        {
            ...
            if ( $version != Q_SUPPORT_VERSION ) {
                ...
                $q_support_options["deferred_admin_notices"][] = __( "Q Support: Upgraded version $version to ".Q_SUPPORT_VERSION.".", Q_Support::$text_domain );
            ...
            // get plugin name ##
                ...
                if ( isset( $_GET["settings-updated"] ) ) {
                    ...
                    if ( isset ( $_GET["page"] ) && $_GET["page"] === 'settings' ) {
                        ...
                        $notices[] = __("Q Support Settings Saved.", Q_Support::$text_domain); // add saved message ##
