...
/**
* Poll Q Support to get latest data for questions
* 
* @since        0.4
* @return       void
*/
function q_support_poll()
{
    ...
    foreach( $questions as $question ) {
        ...
        if ( $question_key = get_post_meta( $question, 'q_support_question_key', true ) ) {
            
           $question_keys[$question] = $question_key; 
    ...
    // store it ##
    #Q_Plugin::$store['q_support_poll']["q_support_questions"] = var_export ( $question_keys, true );
    
    // check if the array of question IDs is empty - array_filter without args drops all null, '', 0 values... ##
    if ( empty ( $question_keys ) ) {
        
        // store it ##
        #Q_Plugin::$store['q_support_poll']["no_q_support_questions"] = true;
    ...
    if ( is_wp_error( $result ) ) {
        ...
        if ( isset ( $results->status ) && $results->status === true ) {
            ...
            foreach( $results->response as $key => $question ) {
                ...
                // update resolved status - if isset ##
                if ( isset ( $question->post_resolved ) ) {
                    
                    update_post_meta( $post_id, 'q_support_question_resolved', "1" );

                } else {

                    delete_post_meta( $post_id, 'q_support_question_resolved' );
